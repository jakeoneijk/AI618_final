...
class VCTKMultiSpkDataset(Dataset):
    def __init__(self, hparams, cv=0):  #cv 0: train, 1: val, 2: test
        ...
        if self.cv == 2 : 
            print('test split')
            self.data_list = sorted(glob(path.join(self.directory, 'test', self.dataformat)))
            print([os.path.basename(t) for t in self.data_list])
        else :         
...
class VCTKSingleSpkDataset(Dataset):
    def __init__(self, hparams, cv=0):  # cv 0: train, 1: val, 2: test
        ...
        if self.cv == 2 : 
            print('test split')
            audio_list = sorted(glob(path.join(folder, 'test', file_format)))
            print(audio_list)
        else : 
